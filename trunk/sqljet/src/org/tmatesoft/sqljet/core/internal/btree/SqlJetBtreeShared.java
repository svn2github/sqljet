...
public class SqlJetBtreeShared {
     ...
     */
     private void ptrmapGet(int key, byte[] pEType, int[] pPgno) throws SqlJetException{
       ...
       if( pPgno!=null && pPgno.length>0 ) pPgno[0] = 
           SqlJetUtility.get4byte(pPtrmap, offset+1);
     ...
     */
     public SqlJetMemPage allocateBtreePage( 
             int[] pPgno, int nearby, boolean exact ) throws SqlJetException
     {
  ...
  try {
       if( n>0 ){
         ...
         int iTrunk;
         boolean searchList = false; /* If the free-list must be searched for 'nearby' */
         ...
         ** end of the file */
         ...
         if( autoVacuum && PTRMAP_ISPAGE(pPgno[0]) ){
           ...
           */
           ...
           pPgno[0]++;
           if( pPgno[0]==PENDING_BYTE_PAGE() ){ pPgno[0]++; }
         }
     ...
     * @param eType Pointer map 'type' entry for pDbPage
     * @param iPtrPage Pointer map 'page-no' entry for pDbPage
     * @param iFreePage The location to move pDbPage to
     * @param isCommit
     */
    private void relocatePage(
       SqlJetMemPage pDbPage, byte eType, int iPtrPage, int iFreePage, 
       boolean isCommit ) throws SqlJetException
    {
       /* The page that contains a pointer to pDbPage */        
       SqlJetMemPage pPtrPage;   
       int iDbPage = pDbPage.pgno;

       assertion( eType==PTRMAP_OVERFLOW2 || eType==PTRMAP_OVERFLOW1 || 
